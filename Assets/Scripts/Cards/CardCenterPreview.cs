...
// Shows a card enlarged ("unfolded") in the center of the screen — the other half of the token
// presentation: a TokenCard is a small round chip until you hover it, at which point it blooms into
// a readable card here. Ported from Runeboard with two changes: the card prefab to clone is a
// serialized field instead of a DeckManager lookup, and ShowPreviewForCharacter (which walked a
// Character's army to preview its troop cards) is gone with the rest of the gameplay layer — the
// multi-card path it used remains, so pass your own list to get the same row layout.
public partial class CardCenterPreview : MonoBehaviour
{
    [AutoStaticsCleanup]
    public static CardCenterPreview Instance { get; private set; }

    [Header("Card Template")]
    [Tooltip("Card prefab cloned for each previewed card. Assign Card.prefab.")]
    [SerializeField] private GameObject cardPrefab;

    [Header("Center Preview")]
    [Tooltip("Optional RectTransform the preview is parented under. If unassigned, the preview is centered on this object.")]
    ...
    [Tooltip("For hover-triggered previews only (see ShowPreview's hoverDriven param): once the " +
        ...
        "deactivated hover target, or a missed OnPointerExit). Scripted (non-hover) previews are " +
        "exempt so an incidental mouse move can't cut them short.")]
    ...
    // Resolved at runtime so newly-added serialized fields left at 0 still animate instead of
    // leaving the preview stuck invisible at alpha 0.
    ...
    // Above the gameplay UI it is triggered from. Its own Canvas with overrideSorting is what makes
    // that reliable regardless of where in the hierarchy this object was placed.
    ...
    // Shows several cards side by side. A single entry behaves identically to ShowPreview(CardData) —
    // same centered position and scale.
    // hoverDriven marks this as a hover-triggered preview: once shown, if the mouse strays
    // hoverAutoHideDistance pixels from where it was at that moment, the preview force-hides itself
    // even if the hover source never calls HidePreview (see Update's safety net). Leave false for
    // scripted/timed reveals that aren't tied to the cursor sitting still.
    // worldAnchor: when supplied by a caller whose own tokens fan out around a world-space point,
    // the preview is centered on that point instead — otherwise the enlarged card and the tokens it
    // bloomed from end up anchored to two unrelated screen positions.
    public void ShowPreview(IReadOnlyList<CardData> cardsData, float speedMultiplier = 1f, bool hoverDriven = false, Vector3? worldAnchor = null, Camera worldAnchorCamera = null, System.Action onClick = null)
    {
        ...
        if (cardPrefab == null)
        {
            Debug.LogWarning($"{nameof(CardCenterPreview)} on '{name}' has no cardPrefab assigned — nothing to clone.", this);
            return;
        }
        ...
        // Keep generated cards beneath this object's dedicated override-sorting Canvas. Parenting
        // them to the root canvas would escape it, so sortingOrder could no longer put previews in
        // front of whatever triggered them.
        Transform parent = centerPreviewAnchor != null ? (Transform)centerPreviewAnchor : transform;
        ...
        float cardWidth = cardPrefab.TryGetComponent(out RectTransform templateRect) ? templateRect.rect.width : 0f;
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (previewCard != null)
            {
                // The center preview must always show official card art, never a same-named sprite
                // from another art folder that the default lookup would also search.
    ...
    {
        float backdropTarget = previewActive ? BackdropMax : 0f;
    ...
    // Positions/scales/fades every active preview card at intro progress t (0 = fly-in start,
    // 1 = settled), each relative to its own resting position in the row.
    ...
    {
        RectTransform reference = centerPreviewAnchor != null ? centerPreviewAnchor : transform as RectTransform;
        return reference != null ? reference.rect.width : 0f;
    }

    // Relocates the user-assigned backdrop to sit as a full-screen sibling immediately beneath the
    // preview, so it reliably renders behind the card and dims the screen, regardless of where it
    // originally lived in the hierarchy.
    ...
    // The clone's own Card component (SuppressHoverEffects=true) would otherwise handle a click
    // itself and re-evaluate playability against its own stale state — it isn't the token that
    // already computed that. A full-rect catcher on top, forwarding to the caller's onClick, routes
    // the click to the real token's already-known state instead.
