...
var hero=Card("Hero","Character",2,2); hero.startingPC="Elsewhere"; r.Players[0].Hand.Add(hero); Check(!r.Play(hero),"Character away from its home accepted"); hero.startingPC="Home"; Check(!r.Play(hero)&&r.Message.Contains("entered"),"Character recruited at a town nobody entered"); Check(r.Stage==MatchStage.Muster&&r.CanEnter(ready)&&r.Enter(ready)&&ready.Tapped&&r.Players[0].Destination.Entered,"Entering the town from Muster failed"); Check(r.Play(hero),"Character deployment failed"); Check(r.Players[0].Field.Find(u=>u.Card==hero).Tapped,"A recruit must enter tapped");
var hu=r.Players[0].Field.Find(u=>u.Card==hero); var item=Card("Relic","Object"); item.objectType=ObjectTypeEnum.Jewel; pc.objectTypes.Add(ObjectTypeEnum.Jewel); r.Players[0].Hand.Add(item); Check(!r.Play(item),"Object played without character"); Check(r.Play(item,hu)&&hu.Objects.Contains(item),"Second play at an entered destination refused");
...
Check(ready.Tapped&&!r.CanAttack(ready),"A unit that entered a town stays tapped through the enemy's next road"); ready.Tapped=false;
...
var battle=r.LastBattle; Check(battle!=null&&battle.Kind==MatchRules.BattleKind.Road&&battle.Player==1&&battle.Region=="Far"&&battle.Clashes.Count==1&&battle.Clashes[0].Attacker==ready&&battle.Clashes[0].Blockers.Count==2&&battle.Clashes[0].Fights.Count==2&&battle.Clashes[0].Unblocked==0,"Battle report wrong");
...
return "PASS: refill to 7, Halted, stage gates, land-gated destinations, entering a town then recruiting at home, event safety, road attacks with summon timing, attack tap, multiple blockers, simultaneous damage, the battle report, spoils and handoff.";
