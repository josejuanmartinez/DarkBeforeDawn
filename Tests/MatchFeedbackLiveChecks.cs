...
System.Collections.IEnumerator Run() {
    ...
    Check(m.AdvanceIfNoActions()&&r.Stage==MatchStage.Attack,"Empty Events did not auto-skip");yield return null;
    var vb=View(beast);
    Check(vv.ActionHighlighted&&!vb.ActionHighlighted&&!r.Players[0].Field.First(u=>u.Card==beast).Tapped,"New/ready attacker highlights wrong");
    Check(m.InspectionHint(vb).Contains("next turn"),"New unit explanation missing");
    Check(m.Select(vv)&&veteran.Tapped,"Veteran did not tap to attack");
    Check(m.AdvanceIfNoActions()&&r.Stage==MatchStage.Defend,"Exhausted attack stage stuck");
    Check(m.AdvanceIfNoActions()&&r.Stage==MatchStage.Spoils,"No defenders did not auto-resolve");
